...
/**
 * Reverses a money transfer and deletes the record.
 */
export const deleteTransfer = async (userId, transferId, fromCardId, toCardId, amount) => {
    try {
        await runTransaction(db, async (transaction) => {
            const transferRef = doc(db, `users/${userId}/${collections.transfers}`, transferId);
            const fromRef = doc(db, `users/${userId}/${collections.cards}`, fromCardId);
            const toRef = doc(db, `users/${userId}/${collections.cards}`, toCardId);
