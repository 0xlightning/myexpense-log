...
import { Wallet, ArrowUpRight, ArrowDownRight, CreditCard, Activity, TrendingUp, TrendingDown, Banknote, Briefcase, AlertCircle, Plus, Minus, PiggyBank } from 'lucide-react';
...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (currentUser) {
            let mounted = true;
            let loadedCount = 0;
            const requiredDataSources = 5;
            const trackers = { ledger: false, cards: false, categories: false, budgets: false, expenses: false };
            ...
            const unsubBudgets = subscribeToCollection(currentUser.uid, collections.budgets, (data) => {
                setBudgets(data);
                checkLoading('budgets');
            }, handleError);

            const unsubExpenses = subscribeToCollection(currentUser.uid, collections.expenditure_records, (data) => {
                setExpenses(data);
                checkLoading('expenses');
            }, handleError);
    ...
    // --- Aggregations ---
    // BRD §8.2: Net Worth = sum(Debit/Cash) - sum(Credit owed) + sum(Investment values)
    const netWorth = cards.filter(c => c.isActive !== false).reduce((sum, c) => {
        if (c.type === 'credit') {
            // Credit balance represents money owed (stored as negative), so we add it (subtracts net worth)
            ...
            {activeTab === 'all' && (
                <div className="space-y-8">
                    ...
                    {budgets.length > 0 && (() => {
                        ...
                        const topBudgets = [...budgets].slice(0, 5).map(b => ({
                            ...b,
                            spent: monthSpend[b.categoryId] || 0,
                            pct: Math.min(((monthSpend[b.categoryId] || 0) / (b.limitAmount || 1)) * 100, 100),
                            catName: categories.find(c => c.id === b.categoryId)?.name || 'Other',
                        }));
                        return (
                            <Card className="p-8">
                                <div className="flex items-center justify-between mb-6">
                                    <h3 className="text-base font-bold text-slate-900 uppercase tracking-widest flex items-center gap-2">
                                        <PiggyBank size={18} className="text-teal-600" /> Budget Health
                                    </h3>
                                    <a href="/budgeting" className="text-[10px] font-bold text-teal-600 hover:underline uppercase tracking-widest">View All →</a>
                                </div>
                                <div className="space-y-4">
                                    {topBudgets.map((b, i) => (
                                        <div key={i} className="space-y-1.5">
                                            <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest">
                                                <span className="text-slate-500">{b.catName}</span>
                                                <span className={b.pct >= 100 ? 'text-rose-600' : b.pct >= 80 ? 'text-amber-600' : 'text-teal-600'}>
                                                    ${b.spent.toLocaleString()} / ${(b.limitAmount || 0).toLocaleString()}
                                                </span>
                                            </div>
                                            <div className="h-2 w-full bg-stone-100 rounded-full overflow-hidden">
                                                <div
                                                    className={`h-full rounded-full transition-all duration-700 ${b.pct >= 100 ? 'bg-rose-500' : b.pct >= 80 ? 'bg-amber-500' : 'bg-teal-500'}`}
                                                    style={{ width: `${b.pct}%` }}
                                                />
                                            </div>
                                        </div>
