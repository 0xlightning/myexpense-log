...
export default function CardPage() {
    ...
    const getTypeIcon = (t) => {
        switch (t) {
            case 'credit': return <CreditCard className="text-amber-500" size={24} />;
            case 'cash': return <Banknote className="text-green-500" size={24} />;
            case 'debit': return <CreditCard className="text-teal-600" size={24} />;
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-stone-900 tracking-tight flex items-center gap-3 uppercase">
                        <Landmark className="text-teal-600" size={28} />
                        Payment Methods
                    </h1>
                    <p className="text-stone-500 mt-1 font-medium">Manage your accounts, cards, and wallets with precision.</p>
                </div>
                <Button onClick={handleAddNew} className="bg-teal-600 hover:bg-teal-700 text-white shadow-lg shadow-teal-200/50 px-6 h-[42px] rounded-xl flex items-center gap-2 w-full md:w-auto justify-center">
                    <Plus size={18} strokeWidth={3} />
                    <span className="font-bold uppercase text-xs tracking-wider">Add Method</span>
                </Button>
            </div>
                    ...
                    <Card key={card.id} className="group relative p-8 h-full flex flex-col">
                        <div className="relative z-10 flex-1">
                            <div className="flex justify-between items-start mb-8">
                                <div className={`p-4 rounded-xl ${card.type === 'credit' ? 'bg-amber-50 text-amber-600' : card.type === 'cash' ? 'bg-lime-50 text-lime-600' : 'bg-stone-100 text-teal-600'} border border-stone-100`}>
                                    {getTypeIcon(card.type)}
                                </div>
                                <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${card.type === 'credit' ? 'border-amber-200 bg-amber-50 text-amber-600' : card.type === 'cash' ? 'border-lime-200 bg-lime-50 text-lime-600' : 'border-teal-600/20 bg-stone-100 text-teal-600'}`}>
                                    {card.type}
                                </div>
                            </div>

                            <h3 className="text-xl font-black text-stone-800 mb-2 tracking-tight group-hover:text-teal-600 transition-colors uppercase whitespace-nowrap overflow-hidden text-ellipsis">{card.name}</h3>
                            ...
                            {card.type === 'credit' && (
                                <div className="space-y-3 mb-6 bg-stone-50/50 p-4 rounded-xl border border-stone-100/50">
                                    <div className="flex justify-between text-[10px] font-bold text-stone-400 uppercase tracking-widest">
                                        <span>Utilized</span>
                                        <span className={
                                            Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 80
                                                ? 'text-rose-600'
                                                : Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 30
                                                    ? 'text-amber-600'
                                                    : 'text-teal-600'
                                        }>
                                    ...
                                    <div className="h-1.5 w-full bg-stone-200 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all duration-1000 ${Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 80
                                                    ? 'bg-rose-500'
                                                    : Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 30
                                                        ? 'bg-amber-500'
                                                        : 'bg-teal-600'
                                    ...
                                    {/* BRD §8.7: Alert if utilization > 30% */}
                                    {Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 30 &&
                                        Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) <= 80 && (
                                            <p className="text-[9px] font-bold text-amber-600 uppercase tracking-widest flex items-center gap-1">
                                                ⚠ Utilization above 30% — keep below for better credit health.
                                            </p>
                                        )}
                                    {Math.abs(((card.balance || 0) / (card.creditLimit || 1)) * 100) > 80 && (
                                        <p className="text-[9px] font-bold text-rose-600 uppercase tracking-widest flex items-center gap-1">
                                            ⛔ High utilization! Exceeds 80% of limit.
                                        </p>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="mt-auto pt-6 border-t border-stone-100 flex gap-3 md:opacity-0 group-hover:opacity-100 transition-all duration-300">
                            <Button onClick={() => handleEdit(card)} className="flex-1 bg-white hover:bg-stone-50 text-stone-600 border-stone-200 rounded-xl py-2 text-[10px] font-bold uppercase tracking-widest shadow-sm">
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest ml-1">Account / Card Name</label>
                        <Input
                            required
                            value={name}
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest ml-1">Method Type</label>
                        <select
                        ...
                        >
                            <option value="bank" className="bg-white text-stone-900 font-medium">Bank Account</option>
                            <option value="debit" className="bg-white text-stone-900 font-medium">Debit Card</option>
                            <option value="credit" className="bg-white text-stone-900 font-medium">Credit Card</option>
                            <option value="cash" className="bg-white text-stone-900 font-medium">Cash / Wallet</option>
                        </select>
                    </div>
                    ...
                    {!editingCard && (
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <Banknote className="absolute left-4 top-1/2 -translate-y-1/2 text-lime-600" size={18} />
                                <Input
                                    type="number"
                                    step="0.01"
                    ...
                    {type === 'credit' && (
                        <div className="space-y-2">
                            <label className="text-[10px] font-bold text-stone-400 uppercase tracking-widest ml-1">Credit Limit</label>
                            <div className="relative">
                                <CreditCard className="absolute left-4 top-1/2 -translate-y-1/2 text-amber-600" size={18} />
                                <Input
                                    type="number"
                                    step="0.01"
                                    value={creditLimit}
