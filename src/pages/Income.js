        ...
        <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Track your earnings and deposits.</p>
                </div>
                <Tabs
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-2xl p-6 shadow-xl shadow-slate-200/50 border border-slate-100 sticky top-8">
                            <h2 className="text-lg font-bold text-slate-900 mb-6">Manage Sources</h2>
                            <form onSubmit={handleSourceSubmit} className="space-y-4">
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-1.5 ml-1">Source Name</label>
                            ...
                            <div className="mt-8 space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Your Sources</h3>
                                {sources.map(source => (
                                    <div key={source.id} className="group flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 hover:border-emerald-200 hover:bg-emerald-50/30 transition-all">
                                        <span className="font-medium text-slate-700">{source.name}</span>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button onClick={() => { setEditingSource(source); setSourceName(source.name); }} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-white rounded-lg"><Edit3 size={14} /></button>
                                            <button onClick={() => handleSourceDelete(source)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-white rounded-lg"><Trash2 size={14} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
                            ...
                            <form onSubmit={handleItemSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-1.5">
                                    <label className="text-sm font-semibold text-slate-700 ml-1">Income Source</label>
                                    <select
                                        required
                                        value={sourceId}
                                        onChange={(e) => setSourceId(e.target.value)}
                                        className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all"
                                    >
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-sm font-semibold text-slate-700 ml-1">Deposit To</label>
                                    <div className="relative">
                                        ...
                                        <select
                                            ...
                                            className="w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 py-3 text-slate-900 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all disabled:bg-slate-100"
                                        >
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input
                                            ...
                                            disabled={editingItem}
                                            className="pl-11 bg-slate-50 border-slate-200 font-semibold text-lg text-emerald-700"
                                            placeholder="0.00"
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="md:col-span-2 flex gap-3 mt-4">
                                    <Button type="submit" disabled={loading} className="flex-1 bg-slate-900 hover:bg-black text-white h-12 rounded-xl text-base font-bold shadow-lg shadow-slate-900/10 transition-all hover:-translate-y-0.5 active:scale-[0.98]">
                                        {loading ? 'Processing...' : (editingItem ? 'Update Record' : 'Confirm Deposit')}
                                    </Button>
                                    {editingItem && (
                                        <Button type="button" variant="secondary" onClick={() => { setEditingItem(null); setSourceId(''); setCardId(''); setAmount(''); }} className="px-6 rounded-xl font-semibold">
                                            Cancel
                                        </Button>
                                    )}
                                </div>
            ...
            {activeTab === 'source' && (
                <div className="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
                    <div className="flex items-center justify-between mb-8">
                        <h2 className="text-xl font-extrabold text-slate-900 tracking-tight">Transaction History</h2>
                        <Button
                        ...
                        >
                            <div className="bg-white/20 p-1 rounded-full group-hover:rotate-90 transition-transform duration-300">
                                <Plus size={16} className="text-white bg-emerald-400 rounded-sm" />
                            </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 text-slate-500">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-semibold">Deposited To</th>
                                    ...
                                    <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleItemEdit(item)} className="p-2 text-slate-400 hover:text-indigo-600 bg-white border border-transparent hover:border-slate-200 rounded-lg shadow-sm transition-all">
                                                    <Edit3 size={14} />
                                                </button>
                                                <button onClick={() => handleItemDelete(item)} className="p-2 text-slate-400 hover:text-rose-600 bg-white border border-transparent hover:border-slate-200 rounded-lg shadow-sm transition-all">
                                                    <Trash2 size={14} />
                                                </button>
                                            </div>
                                        </td>
